...
test.describe('Booking Management — Critical Happy Paths', () => {
  ...
  test('TC-006: navigates to bookings list after booking via View My Bookings link', async ({ page }) => {
    ...
    // (bookEvent() leaves the page on the confirmation card, so no extra
    // navigation/re-booking is needed here — reuses the shared helper instead
    // of re-implementing the booking form fill inline.)
  ...
  test('TC-102: booking reference starts with first letter of event title (uppercase)', async ({ page }) => {
    ...
    // Plain string checks instead of a dynamic RegExp: event titles are
    // live/seeded data and can legitimately start with regex metacharacters
    // (e.g. "(", ".", "*" — see docs/test-scenarios.md TC-407), which would
    // make `new RegExp(`^${expectedPrefix}-...`)` throw instead of failing cleanly.
    const expectedPrefix = eventTitle[0].toUpperCase();
    expect(bookingRef[0]).toBe(expectedPrefix);
    expect(bookingRef.slice(1)).toMatch(/^-[A-Z0-9]{6}$/);
